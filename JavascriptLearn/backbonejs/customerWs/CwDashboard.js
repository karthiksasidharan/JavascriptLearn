...
function displayLifeCycleStatus1(data){
                var sum=0;
				var i=0;
                for(var lc in data){
					if(i<6){
							sum+=parseInt(data[lc]);
							}
					i++;
                }
                $( ".progress" ).each(function( index ) {
                        ...
                        if(data[id]==null){
                                ...
                                var height=progressContainerHeight;
								var calcPercent =0;
								if(sum>0){
                                 calcPercent = (data[id]/sum)*progressContainerHeight;
								}
                                var currObj = $(this).find(".progress-vertical")[0];
                                applyChartColor(currObj,calcPercent)
                                console.log(height)
                                $("#"+id+"_label").html(data[id]);
                                $(this).find(".progress-vertical").css("height",progressContainerHeight-70+"px");
                                $(this).height(progressContainerHeight-50);
                        }
                });
...
var colSelectionWindow = (function () {
	...
	function _addEventListeners() {
		$('#resetProjColumns').click(function () {
			console.log('event fired...');
			$("#projColumnChoice").jqxListBox('checkAll');
        });
	};
...
function setDatePicker(){
	var optionSet2 = {
			...
			ranges: {
				...
				'All':[moment().subtract(1500,'days'),moment()]
			}
	};
	...
	$('.widgetDatePicker span').html(moment().subtract(1825, 'days').format('MMM D, YYYY') + ' - ' + moment().format('MMM D, YYYY'));
	$('#LcStatusRange span').html(moment().subtract(89, 'days').format('MMM D, YYYY') + ' - ' + moment().format('MMM D, YYYY'));

	$('#qualityDashboardRange span').html(moment().subtract(364, 'days').format('MMM D, YYYY') + ' - ' + moment().format('MMM D, YYYY'));

	$('#relTrendingRange span').html(moment().subtract(364, 'days').format('MMM D, YYYY') + ' - ' + moment().format('MMM D, YYYY'));

	lcStartDate=moment().subtract(90, 'days').format('DD-MM-YYYY');
	...
	asBuStartDate=moment().subtract(365, 'days').format('DD-MM-YYYY');
	asBuEndDate=moment().format('DD-MM-YYYY');
...
var browseProjectsClass = (function(){
	...
	function constructBrowseProject(data){
		...
		for(var i in data){
			var projUsage = data[i]["projUsage"];
			data[i]["projUsage"] = getUsage(projUsage);

			var osType = data[i]["osType"];
			data[i]["osType"] = getOsType(osType);

			var lifeCycleStatus = data[i]["lifeCycleStatus"];
			data[i]["lifeCycleStatus"] = lcMap[lifeCycleStatus] != null ||  lcMap[lifeCycleStatus] != undefined ?
					lcMap[lifeCycleStatus] : "-";

					if(i == 0 && data[i]["projId"] == getRequestedProjId()){

						//console.log(data[i]);
						
						if(data[i]["projName"]){
							data[i]["projName"] = "<b>"+data[i]["projName"]+"</b>";
							data[i]["projName"] = data[i]["projName"].replace(/<a/,"<a style=\"color:blue !important;\"");	
							}
						
						
						if(data[i]["priEngr"]){
						data[i]["priEngr"] = "<b style=\"color:blue\">"+data[i]["priEngr"]+"</b>";	
						}
						
						if(data[i]["deliveryEngr"]){
						data[i]["deliveryEngr"] = "<b style=\"color:blue\">"+data[i]["deliveryEngr"]+"</b>";	
						}
						
						if(data[i]["startRel"]){
						data[i]["startRel"] = "<b style=\"color:blue\">"+data[i]["startRel"]+"</b>";
						}
						
						if(data[i]["projUsage"]){
						data[i]["projUsage"] = "<b style=\"color:blue\">"+data[i]["projUsage"]+"</b>";	
						}
						
						if(data[i]["osType"]){
						data[i]["osType"] = "<b style=\"color:blue\">"+data[i]["osType"]+"</b>";	
						}
						
						if(data[i]["duId"]){
						data[i]["duId"] = "<b style=\"color:blue\">"+data[i]["duId"]+"</b>";	
						}
						
						if(data[i]["projDesc"]){
							data[i]["projDesc"] = "<b style=\"color:blue\">"+data[i]["projDesc"]+"</b>";	
						}
						if(data[i]["lastModified"]){
						data[i]["lastModified"] = "<b style=\"color:blue\">"+data[i]["lastModified"]+"</b>";	
						}
						
						if(data[i]["reportType"]){
						data[i]["reportType"] = "<b style=\"color:blue\">"+data[i]["reportType"]+"</b>";
						}
						
						if(data[i]["searchTyp"]){
							data[i]["searchTyp"] = "<b style=\"color:blue\">"+data[i]["searchTyp"]+"</b>";
						}
							
						if(data[i]["workflowStatus"]){
						data[i]["workflowStatus"] = "<b style=\"color:blue\">"+data[i]["workflowStatus"]+"</b>";	
						}
						if(data[i]["rptSts"]){
						data[i]["rptSts"] = "<b style=\"color:blue\">"+data[i]["rptSts"]+"</b>";	
						data[i]["rptSts"] = data[i]["rptSts"].replace(/<a/,"<a style=\"color:blue !important;\"");
						}
						
						if(data[i]["lifeCycleStatus"]){
						data[i]["lifeCycleStatus"] = "<b style=\"color:blue\">"+data[i]["lifeCycleStatus"]+"</b>";	
						}
						
						if(data[i]["otherOsTypeName"]){
							data[i]["otherOsTypeName"] = "<b style=\"color:blue\">"+data[i]["otherOsTypeName"]+"</b>";
						}

						if(data[i]["nxosPlatform"]){
							data[i]["nxosPlatform"] = "<b style=\"color:blue\">"+data[i]["nxosPlatform"]+"</b>";
						}
						
						if(data[i]["prodFamily"]){
							data[i]["prodFamily"] = "<b style=\"color:blue\">"+data[i]["prodFamily"]+"</b>";
						}
					...
					columns: [
					          ...
					          { text: 'Target Release', datafield: 'startRel', width: 100, cellsrenderer : function (row, column, cellvalue){
					        	  ...
					        	  { text: 'Other OS name', datafield: 'otherOsTypeName', width: 100, cellsrenderer : function (row, column, cellvalue){
					        		  ...
					        		  { text: 'Life Cycle State', datafield: 'lifeCycleStatus', width: 100},
					        		  { text: 'Project Usage', datafield: 'projUsage', width: 100},
		...
		clearProjSelection();//clear any selected rows
	...
	function clearProjSelection(){
		var rowcount = $("#browseProjectDiv").jqxGrid('getdatainformation').rowscount;

		for(var i=0; i<rowcount; i++)
		{
						$("#browseProjectDiv").jqxGrid('unselectrow',i);
	...
	function validateClickForProject(){
		...
		if (getselectedrowindexes.length ==1){
			...
			alert("Please Select a Single Project");
			return 0;
		}else{
	...
	return{
		...
		browseProject:function (startDate,endDate){
			...
			var success = function(data){
				...
				constructBrowseProject(copiedData);
		...
		cloneProject: function (){
			...
			var tmpProjId= projectsData[i]["projId"];
			$("#cloneDialog").dialog({
                height:550,
                width:1000,
                title:'Clone Project',
                ...
                closeOnEscape:true,
                open: function(ev, ui){
                	$('#cloneProjLoader').show();
                	$('#cloneProjLoader').css("top","800px");
                	$('#cloneProjLoader').css("z-index","9000");
                	$('#cloneProjLoader').css("left","50%");
                	$('#cloneProjLoader').css("position","absolute");
                    $('#myIframe').attr('src',"/SORAServices/project/clone.htm?projectId="+tmpProjId);
                 },
		...
		mergeProjects:function(){
			...
			if(customerId) {
				urlArgs = urlArgs +"&customerId="+customerId;
			}
			var success= function(data){
		...
		hardwareSelector:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/profile/ProfileScreen.htm?projectId='+projId,'_blank');
			}
		},
		...
		componentSelector:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/filter/ComponentSelector.htm?projectId='+projId,'_blank');
			}
		},
		queryScreen:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/filter/OptionalFilter.htm?projectId='+projId,'_blank');
			}
		},
		componentSummary:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/component/ComponentSummary.htm?projectId='+projId,'_blank');
			}
		},
		...
		bugList:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/buglist/BugList.htm?projectId='+projId,'_blank');
			}
		},
		...
		brp:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/brp/BRPMain.htm?projectId='+projId,'_blank');
			}
		},
		...
		reportGeneration:function(){
			...
			if(projId==0){
				...
				window.open('/SORAServices/report/ReportGen.htm?projectId='+projId,'_blank');
			}
		},
...
var periodicBugs = (function(){
	...
	return{
		...
		getDetails:function(row,filter){
			...
			var custNameTemp = selectedCustName != "" ? selectedCustName : customerNameModal;
			var param = "custId="+customerId+"&osType="+osType+"&found="+found+"&severity="+severity+"&custName="+custNameTemp;
		...
		getPeriodicProjectData:function(){
			...
			success = function(data){
				...
				if(data.countData == null || data.countData.length == 1){
					$('#nodata').html("No Non-Analysed Periodic Bugs present. ");
					$('#data').hide();
					$('#nodata').show();
				}else{
					$('#nodata').hide();
					...
					$("#data").jqxGrid(
							{
								...
								autoheight: true,
								columns: [
								          { text: 'OS Type', datafield: 'osType', width: '15%' },
								          { text: 'S-1,2', datafield: 'severityOneTwoCount', width: '9%',cellsrenderer: cellsrenderer },
								          { text: 'S-1,2,3', datafield: 'severityOneTwoThreeCount', width: '9%' ,cellsrenderer: cellsrenderer},
								          { text: 'S-1', datafield: 'severityOneCount', width: '9%', cellsrenderer: cellsrenderer },
								          { text: 'S-2', datafield: 'severityTwoCount', width: '9%',cellsrenderer: cellsrenderer},
								          { text: 'S-3', datafield: 'severityThreeCount', width: '9%',cellsrenderer: cellsrenderer },
								          { text: 'S-4,5,6', datafield: 'severityFourFiveSixCount', width: '10%',cellsrenderer: cellsrenderer },
								          { text: 'Customer Found', datafield: 'customerFoundCount', width:'15%' ,cellsrenderer: cellsrenderer},
								          { text: 'Internal Found', datafield: 'internalFoundCount', width: '15%' ,cellsrenderer: cellsrenderer}
...
var qualityDashboardClass = (function(){
	...
	return{
		...
		getAsBuProdFamily:function(startDate,endDate){
			$.ajax({
				url:"/SORAServices/customerWs/getAsBuPodFamily.json?startDate="+asBuStartDate+"&endDate="+asBuEndDate+"&custId="+customerId,
				type:"GET",
				success:function(data){
	...
	function displayNCEActivityData(actList) {
		...
		if(actList.length>0){
			for(var i=0; i<actList.length; i++){
				...
				if(actList[i].release && actList[i].release != undefined){
					relString = " for "+ actList[i].release;
				}
				if (str.contains("-")){
					var a= str.indexOf("-");
					var b = str.indexOf("(");
					...
					nceMediaBody="<div class='row' style='display:inline-block; width:100%'><div class='col-lg-1'>" +
							"<a class='pull-left' href='#'>" +
							"<img style='height:44px;width:33px;'class='media-object' src='http://wwwin.cisco.com/dir/photo/std/" +
							actList[i].usrId + ".jpg'>" +
							"</a>" +
							"</div>" +
							"<div class='col-lg-8' style='font-size:12px; word-wrap:break-word; padding-top:5px;' class='media-body1'>" +
							"<a href='http://wwwin-tools.cisco.com/dir/details/"+actList[i].usrId +"' ondata-toggle='tooltip' style='font-size:12px;'>"+
							res1+
							"</a>"
							+res2+")"+" "+(actList[i].operation).toLowerCase() + relString +" ( "+ actList[i].projName+" ) " +
							"</div>" +
							" <div class='col-lg-3' style='font-size:12px;color:grey;' class='media-body2'>"+
							actList[i].opnSinceTime+
							"</div>" +
							"</div>" +
							"<hr/>";
				...
				else
				{
					...
					nceMediaBody="<div class='row' style='display:inline-block; width:100%' ><div class='col-lg-1'>" +
							"<a class='pull-left' href='#'>" +
							"<img style='height:44px;width:33px;'class='media-object' src='http://wwwin.cisco.com/dir/photo/std/" + actList[i].usrId + ".jpg'>" +
							"</a>" +
							"</div>" +
							"<div class='col-lg-8' style='font-size:12px; word-wrap:break-word; padding-top:5px;' class='media-body1'>" +
							"<a href='http://wwwin-tools.cisco.com/dir/details/"+actList[i].usrId +"' ondata-toggle='tooltip' style='font-size:12px;'>"+
							res3+
							"</a>"+
							" "+res4+")"+" "+(actList[i].operation).toLowerCase() + relString + " ( "+ actList[i].projName+" )</div> " +
							"<div class='col-lg-3' style='font-size:12px;color:grey;' class='media-body2'>"+ actList[i].opnSinceTime+
							"</div>" +
							"</div>" +
							"<hr/>";
					div.append(nceMediaBody);
...
function constructLcTable(data,lcStatus){
	...
	var row="<thead><tr><th width='5%'>Project Name</th><th width='5%'>OS Type</th><th width='5%'>Release</th><th width='10%'>Product Family</th><th width='5%'>Primary Engineer</th><th width='5%'>Delivery Engineer</th><th width='9%'>Life Cycle Status</th><th width='70%'>Justification</th><th width='5%'>Update</th></tr></thead>";
	table.append(row);
	var number = new RegExp('[0-9]+');
	projMap = new Object();
	...
	for(var i in data["projList"]){
		if(i.match(number)){
			...
			row+="<td>"+data["projList"][i]["osName"]+"</td>";
			row+="<td style='word-break:break-all;'>"+data["projList"][i]["startRel"]+"</td>";
			row+="<td>"+data["projList"][i]["prodFamily"]+"</td>";
...
function closeLcModal(){
	alert("Please note only updated projects are saved");
...
function getLcStatusUIStr(lcStatus){
	...
	for(var i in allowedStatusArr){
		console.log(lcMap[allowedStatusArr[i]]);
		lcStatusOption = lcMap[allowedStatusArr[i]];
...
function setJustificationColumn(lcStatusDpdId, idPrefix, projId){
	...
	if(lcStatus=='R'){
		...
		"<tr>"+
		"<td style='width:30%!important;'><input class='form-control' type='text' id='"+idPrefix+"_bugId'></td>"+
...
function loadAllWidgets(){
	...
	getRecProdFamily();
	//getInitialPosition();
	initializeCreateProjectWidget("createProjDiv", true, "createWidgetLoader", customerId, customerNameModal);
}
...
$(document).ready(function() {
	...
	var success = function(data){
		if(data.value!=null&&data.value!=""){
			...
			$("#customer-search").text(decodeURIComponent(data.label));
			loadAllWidgets();
		}else{
			customerSelectionView=new CustomerSelectionView({ el: $("#modal")});
			$('#modal').modal('show');
			if($("#customer-search").text() == ""){
				$('#custOverlayClose').css("display", "none");
			}else{
				$('#custOverlayClose').css("display", "block");
			}
	...
	if(customerIdModal!=null&&customerIdModal!=""){
		...
		loadAllWidgets();
		$('#customer-search').text(customerNameModal);
	}else{
...
//    $('#createProjDiv' + tabIndex).text(data);
//	});
//	};
//
...
//        if(pageIndex == 2){//if create project tab
//		    loadPage('pages/ajax' + pageIndex + '.htm', pageIndex);
//        	initializeCreateProjectWidget("createProjDiv");
//        }
//    });
	//getInitialPosition();

	// show help text only if customer is empty
	if($("#customer-search").text() == ""){
		showHelptext();
	}
...
$(function() {
	$( "#customerSearchModal").autocomplete({
		...
		select: function( event, ui ) {
			$("#customer-search").text(ui.item.label);
	...
	$(function() {
		$( "#customerSearchModal").autocomplete({
			...
			select: function( event, ui ) {
				...
				tempCustId=ui.item.value;
				tempCustName=ui.item.label;
				$("#customerSearchModal").val(ui.item.label);
				return false;// Prevent the widget from inserting the value.
			},
...
function customerSelected(){
	$("#customer-search").text(tempCustName);
	customerId=tempCustId;
	if($("#customer-search").text() == ""){
		$('#custOverlayClose').css("display", "none");
		alert("Please choose a valid customer");
	}else{
		saveCustomerForUser(customerId,tempCustName);
		loadAllWidgets();
		$('#custOverlayClose').css("display", "block");
		$('#modal').modal('hide');
	}
		...
		myseriesGroups.push(  {
			...
			series:  [
			          {  dataField: 'totalCount', displayText: key ,formatFunction: function (value, itemIndex) {
			        	  ...
			        	  if(displayLable){
			        		  return data[key][itemIndex]["releaseLabel"];
			        	  }else{
			        		  return "";
			        	  }
...
lcMap['A']="Accepted";
     ...
     var progressContainerHeight;
	  progressContainerHeight = $("#periodicBugDiv").height()-175;
	  $(window).resize(function(){adjustChartHeight()});
   function adjustChartHeight(){
                $(".progressBarHeight").height(progressContainerHeight-45);
                //$(".progressBarHeight .progress-vertical").css("height",progressContainerHeight + "px");
            }

	   function applyChartColor(obj,percentage){
                var col1="#fff",
                    col2="#31b0d5";
                var t=obj;
                remPercent = 100-percentage;
                t.style.background = "linear-gradient(to bottom,"+col1+" "+remPercent+"%, "+col2+" "+percentage+"%)";
            }
	   ...
	   function showLoader(){
		...
		function showCustomLoader(customText){
			...
			$('#overlay').css("display", "block");
			$('#overlay').css("height",hght+"px");
			document.getElementById("loaderText").innerHTML = customText;
				}
...
function getRequestedProjId(){
	console.log(" requestedUrl ==>"+requestedUrl);

	var requestedProjId = "";
	if(requestedUrl != null && requestedUrl != undefined && requestedUrl != ""){
		var splitArr = requestedUrl.split("projectId=");

		if(splitArr && splitArr.length > 1){

			if($.isNumeric(splitArr[1])){

				requestedProjId = splitArr[1].trim();

			}else if(splitArr[1].trim().length > 0){
				var temp = splitArr[1];
				splitArr = temp.split("&");
				requestedProjId = splitArr[0].trim();
			}

		}

	}
	console.log("requestedProjId ==>"+requestedProjId);
